...
impl Repository {
    ...
    #[instrument]
    pub async fn open(addr: &OsStr) -> Result<Repository> {
        ...
        match addr.split_once(':') {
            ...
            Some(("sqlite", path)) => Ok(Self(RepositoryInner::Sql(SqlRepository::open(path).await?))), 
    ...
    pub async fn run_command(&mut self, cmd: Command) -> Result<()> {
        match &mut self.0 {
            RepositoryInner::Local(repo) => repo.run_command(cmd),
            RepositoryInner::Sql(repo) => repo.run_command(cmd).await,
    ...
    pub async fn accounts(&self) -> Result<Vec<Account>> {
        Ok(match &self.0 {
            ...
            RepositoryInner::Sql(repo) => repo.accounts().await?,
    ...
    pub async fn account(&self, id: Id<Account>) -> Result<Account> {
        Ok(match &self.0 {
            ...
            RepositoryInner::Sql(repo) => repo.account(id).await?,
    ...
    pub async fn transactions(&self, id: Id<Account>) -> Result<Vec<Transaction>> {
        match &self.0 {
            ...
            RepositoryInner::Sql(repo) => repo.transactions(id).await,
